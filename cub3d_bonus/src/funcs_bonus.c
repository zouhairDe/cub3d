...
/*   Updated: 2025/02/06 19:17:35 by mait-lah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_stripe(t_game *game,int x, t_ray *ray)
{
	...
	double PROJECTION_PLANE_DIST  = (WINDOW_WIDTH / 2.0) / tan(FOV/2.0);
...
void	castRays(t_game *game)
{
	...
	double angle = game->player.dir - (FOV / 2.0);
	double centerX = MINIMAP_WIDTH / 2.0;
    double centerY = MINIMAP_HEIGHT / 2.0;
